...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const colorBarSub = subscriptions.data.find((s: any) => (s.metadata as Record<string, string>)?.addon_type === "backroom"
    );
    ...
    const existingLicenseItem = colorBarSub.items.data.find((item: any) => item.price.id === SCALE_LICENSE_PRICE_ID
    );
