        ...
        <TabsContent value="approvals" className="space-y-4 mt-4">
          <Tabs defaultValue="all">
            <TabsList>
              ...
              <TabsTrigger value="super-admins" className="gap-1">
                <Crown className="w-3.5 h-3.5" />
                Super Admins
                {superAdminCount > 0 && <Badge variant="secondary" className="h-5 min-w-5 text-xs px-1.5">{superAdminCount}</Badge>}
              </TabsTrigger>
            </TabsList>
              ...
              {loadingPendingAccounts ? (
                ...
                <Card>
                  <CardContent className="py-12 text-center">
                    ...
                    <p className="text-muted-foreground">No accounts pending approval</p>
                  </CardContent>
                </Card>
              ) : (
            ...
            <TabsContent value="super-admins" className="space-y-3 mt-4">
              {loadingAccounts ? (
                <div className="flex items-center justify-center py-12">
                  <Loader2 className="w-6 h-6 animate-spin text-muted-foreground" />
                </div>
              ) : (
                allAccounts?.filter(a => a.is_super_admin).map(account => (
