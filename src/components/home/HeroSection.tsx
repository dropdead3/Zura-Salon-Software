...
export function HeroSection({ videoSrc, isPreview = false }: HeroSectionProps) {
  ...
  // Publish the active alignment for global overlays (e.g. promo FAB).
  // The rotator publishes its own per-slide value when it takes over below;
  // this branch covers the static (no-slides) hero. Cleared on unmount.
  useEffect(() => {
    if (hasSlides) return; // rotator owns the signal in that case
    publishHeroAlignment(effectiveAlignment);
    return () => clearHeroAlignment();
  }, [effectiveAlignment, hasSlides]);
