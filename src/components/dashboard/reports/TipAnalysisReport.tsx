...
export function TipAnalysisReport({ dateFrom, dateTo, locationId, onClose }: Props) {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      autoTable(doc, {
        ...branding, startY: y,
        head: [['Stylist', 'Total Tips', 'Appointments', 'Avg Tip', 'Tip-to-Revenue %']],
        body: entries.map(e => [
          e.staffName,
          formatNumber(e.totalTips, 'currency'),
          e.appointmentCount.toString(),
          formatNumber(e.avgTipPerVisit, 'currency'),
          `${e.tipToRevenuePercent.toFixed(1)}%`,
        ]),
      });
  ...
  const downloadCSV = () => {
    const rows = [['Stylist', 'Total Tips', 'Appointments', 'Avg Tip', 'Tip-to-Revenue %'], ...entries.map(e => [e.staffName, e.totalTips.toFixed(2), e.appointmentCount.toString(), e.avgTipPerVisit.toFixed(2), e.tipToRevenuePercent.toFixed(1)])];
    const blob = new Blob([rows.map(r => r.join(',')).join('\n')], { type: 'text/csv' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = buildReportFileName('tip-analysis', dateFrom, dateTo, 'csv');
    a.click();
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            {data && <p className="text-sm text-muted-foreground mt-1">Total: {formatNumber(data.totalTips, 'currency')} · Avg tip rate: {data.avgTipPercent.toFixed(1)}%</p>}
          </div>
        ...
        <CardContent>
          {entries.length === 0 ? (
            <p className={tokens.empty.description}>No tip data for this period.</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {entries.map((e, i) => (
                  <TableRow key={i}>
                    ...
                    <TableCell>{formatNumber(e.totalTips, 'currency')}</TableCell>
                    <TableCell>{e.appointmentCount}</TableCell>
                    <TableCell>{formatNumber(e.avgTipPerVisit, 'currency')}</TableCell>
