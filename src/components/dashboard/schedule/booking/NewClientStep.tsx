...
export function NewClientStep({ defaultLocationId, onCreated, onCancel }: NewClientStepProps) {
  ...
  const stylistWorksAtLocation = useCallback((member: any, locId: string) => {
  ...
  const createClient = useMutation({
    ...
    onSuccess: (client) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['booking-clients'] });
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      >
        ...
        <p className="text-xs text-muted-foreground">* Email or phone is required for contact</p>
          ...
          <div className="space-y-2">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button
                  variant="outline"
                  className={cn('w-full justify-start text-left font-normal', !clientSince && 'text-muted-foreground')}
                >
        ...
        <div className="space-y-2">
                    ...
                    {selectedMember ? (
                      <span className="flex items-center gap-2 min-w-0">
                        <Avatar className="h-5 w-5 shrink-0">
                          ...
                          <AvatarFallback className="text-[9px] bg-muted">{selectedInitials}</AvatarFallback>
                        </Avatar>
                  ...
                  >
                    ...
                    <CommandList className="max-h-[min(60vh,420px)] overflow-y-auto overscroll-contain">
                      <CommandEmpty>
                        <div className="py-3 px-3 text-center space-y-2">
                          <p className="text-sm text-muted-foreground">
                            {locationId && !showAllStylists ? 'No stylists at this location' : 'No stylists found'}
                          </p>
                      ...
                      <CommandGroup>
                        ...
                        >
                          <Check className={cn('mr-2 h-4 w-4', !preferredStylistId ? 'opacity-100' : 'opacity-0')} />
          ...
          {stylistAutoCleared && (
            <p className="text-xs text-muted-foreground">Cleared — previous stylist isn't at this location.</p>
          )}
        </div>
      ...
      <div className="border-t border-border bg-background px-4 py-3 flex items-center justify-end gap-2">
        <Button type="button" variant="outline" onClick={onCancel} disabled={createClient.isPending}>
          Cancel
        </Button>
        <Button type="button" onClick={() => handleSubmit()} disabled={!canSubmit || createClient.isPending}>
