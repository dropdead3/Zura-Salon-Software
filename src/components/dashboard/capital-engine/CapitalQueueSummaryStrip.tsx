...
export function CapitalQueueSummaryStrip({ opportunities, activeProjectCount, totalDeployed }: Props) {
  ...
  const kpis = [
    ...
    { label: 'Total Predicted Lift', value: `+${formatCurrency(c(totalLift), { noCents: true })}` },
    { label: 'Capital Deployed', value: formatCurrency(c(totalDeployed), { noCents: true }) },
  ];
    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
      {kpis.map(k => (
        <div key={k.label} className="p-3 rounded-lg bg-muted/30 border border-border/40">
          ...
          <span className="font-display text-lg tracking-wide">{k.value}</span>
        </div>
      ))}
    </div>
  );
