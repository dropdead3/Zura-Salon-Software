...
interface Props {
  onStart: () => void;
  hasProfile: boolean;
  /** Wave 28.11.1 — actual library size (no longer hardcoded). */
  libraryCount?: number;
}

export function PolicySetupBanner({ onStart, hasProfile, libraryCount }: Props) {
  const count = libraryCount ?? 54;
  return (
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="font-sans text-sm text-muted-foreground mt-1 max-w-2xl">
              {hasProfile
                ...
                : `A 4-step setup tells us which of the ${count} recommended policies actually apply to how you operate. Without this, the library shows everything.`}
            </p>
          </div>
        </div>
