...
function friendlyError(status: number, fallback: string): string {
  ...
  if (status === 409) return 'That request needs approval-gated execution. Open the Zura action panel to stage and approve it.';
...
export function useAIAssistant(): UseAIAssistantReturn {
  ...
  const sendMessage = useCallback(async (query: string, conversationHistory: Message[] = [], organizationId?: string, userRole?: string, groundingContext?: GroundingContextPayload, dataContext?: string) => {
    setIsLoading(true);
    setError(null);
    setResponse('');
