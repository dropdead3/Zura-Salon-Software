  ...
  return items.map((item) => {
    ...
    return {
      ...
      suggestedAction: item.recommended_order_qty > 0
        ...
        : 'Review stock levels',
      actionRoute: '/dashboard/admin/color-bar-settings?category=inventory&tab=stock',
  ...
  return items
    ...
    .map((exc) => {
      ...
      return {
        ...
        actionRoute: '/dashboard/admin/color-bar-settings?category=alerts',
...
function buildProfitabilityAlerts(outliers: MarginOutlier[]): ControlTowerAlert[] {
  return outliers.map((o) => ({
    ...
    actionRoute: '/dashboard/admin/color-bar-settings?category=insights',
...
function buildReorderAlerts(forecasts: ProductDemandForecast[]): ControlTowerAlert[] {
  return forecasts.map((f) => ({
    ...
    actionRoute: '/dashboard/admin/color-bar-settings?category=inventory&tab=reorder',
    createdAt: new Date().toISOString(),
  }));
}
...
function buildPOApprovalAlerts(draftPOs: DraftPOAlert[]): ControlTowerAlert[] {
  ...
  if (autoPOs.length > 0) {
    alerts.push({
      ...
      actionRoute: '/dashboard/admin/color-bar-settings?category=inventory&tab=orders',
...
function buildAuditOverdueAlerts(audits: AuditOverdueAlert[]): ControlTowerAlert[] {
  return audits.map((a) => ({
    ...
    actionRoute: '/dashboard/admin/color-bar-settings?section=inventory&tab=counts',
